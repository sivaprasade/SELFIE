...
app.post('/send_completed_mail', async function mailHandler(req, res) {
    //req.Json:mail.sndConfig 
    const sndCfg = req.body;
    const resBody = await mailer.sendMail(sndCfg);
    res.json({ result: resBody })
});
app.post('/send_deleted_mail', async function mailHandler(req, res) {
    //req.Json:mail.sndConfig 
    const sndCfg = req.body;
    const resBody = await mailer.sendMail(sndCfg);
    res.json({ result: resBody })
});
app.post('/send_due_mail', async function mailHandler(req, res) {
    //req.Json:mail.sndConfig 
    const sndCfg = req.body;
    const resBody = await mailer.sendMail(sndCfg);
    res.json({ result: resBody })
